...
export class Renderer {
  ...
  async initAssets(assetNames, tintDefinitions) {
    ...
    this.preparedTints = tintDefinitions.map((tint) => {
  ...
  render(player, map, raycaster) {
    ...
    // Sélectionner le sprite central
    // const selectedSprite = this._selectCentralSprite(player, map.placeables[Math.floor(player.z)], map);
  ...
  _drawRay(rayResult, x, player, playerZ) {
    ...
    for (let i = rayResult.length - 1; i >= 0; i--) {
      ...
      const cellTop =
        (this.height + cellHeight) / 2 - cellHeight + cellHeight * -zOffset + cellHeight * zRest + verticalAdjustement;
      ...
      if (hit.backDistance) {
        ...
        const backCellTop =
          (this.height + backCellHeight) / 2 -
          backCellHeight +
          backCellHeight * -zOffset +
          backCellHeight * zRest +
          verticalAdjustement;

        if (zOffset >= 0 && hit.ceiling) {
          this._drawTexturedColumn(
            x,
            backCellTop,
            cellTop - backCellTop,
            hit.distance,
            this.textures[hit.ceiling.floorTexture],
            hit.offset,
            1,
            hit.ceilingTint,
            isFacingCell,
        ...
        if (zOffset <= 0 && hit.cellInfos) {
          ...
          if (hit.cellInfos.floorTexture && (hit.floorOnly || !hit.cellInfos.stopView)) {
            this._drawTexturedColumn(
              x,
              cellTop + cellHeight,
              backCellTop + backCellHeight - (cellTop + cellHeight),
              hit.distance,
              this.textures[hit.cellInfos.floorTexture],
              hit.offset,
              0,
              hit.floorTint,
              isFacingCell,
      ...
      if (hit.cellInfos) {
        ...
        if (hit.cellInfos.wallTexture && !hit.cellInfos.thinWall && !hit.floorOnly) {
          ...
          this._drawTexturedColumn(
            ...
            blockHeight,
            hit.distance,
            this.textures[hit.cellInfos.wallTexture],
            hit.offset,
            hit.side,
            hit.wallTint,
            isFacingCell,
        ...
        if (hit.thinDistance && hit.cellInfos.wallTexture) {
          ...
          const cellThinTop =
            (this.height + cellThinHeight) / 2 -
            cellThinHeight +
            cellThinHeight * -zOffset +
            cellThinHeight * zRest +
            verticalAdjustement;
  ...
  _drawTexturedColumn(x, top, height, distance, image, texOffset, side, tint, isFacingCell) {
    ...
    //tinting
    if (tint) {
      this.virtualCtx.fillStyle = this.preparedTints[tint];
  ...
  _drawSprites(player, placeables, map) {
    ...
    let closestCenterSprite = undefined;
    let closestCenterDistance = Infinity;

    //SPRITE CASTING
    for (let i = 0; i < placeables.length; i++) {
      //Calculate sprite distances and reset order
      placeableOrders[i] = i;
      const dx = placeables[i].x - player.x;
      const dy = placeables[i].y - player.y;
      spriteDistance[i] = dx * dx + dy * dy;

      // Vérifier si le sprite est à portée de sélection
      if (spriteDistance[i] <= this.maxSelectionDistance * this.maxSelectionDistance) {
        ...
        if (transformY > 0) {
          const spriteScreenX = centerX * (1 + transformX / transformY);
          const distanceFromCenter = Math.abs(spriteScreenX - centerX);

          if (distanceFromCenter < closestCenterDistance) {
            closestCenterSprite = placeables[i];
            closestCenterDistance = distanceFromCenter;
          }
        }
      }
    }
    this.selectedSprite = closestCenterSprite;
    ...
    for (let i = 0; i < placeables.length; i++) {
      ...
      if (transformY > 0 && transformY < 10) {
        //No need for the rest if the sprite is behind the player
        ...
        if (clipStartX != clipEndX && clipStartX < this.resolution && clipEndX > 0) {
          //Make sure the sprite is not fully obstructed or off screen
          ...
          this.virtualCtx.drawImage(
            placeableSprite.image,
            drawXStart,
            0,
            Math.round(drawXEnd),
            placeableSprite.height,
            Math.round(clipStartX * this.spacing),
            Math.round(drawStartY + verticalAdjustement),
            Math.round(drawWidth * this.spacing),
            Math.round(spriteHeight),
          );
          if (placeables[placeableOrders[i]] === this.selectedSprite) {
            // Dessiner un contour ou un indicateur autour du sprite sélectionné
            this.virtualCtx.strokeStyle = "yellow";
            this.virtualCtx.lineWidth = 2;
            this.virtualCtx.strokeRect(
              Math.round(clipStartX * this.spacing),
              Math.round(drawStartY + verticalAdjustement),
              Math.round(drawWidth * this.spacing),
              Math.round(spriteHeight),
